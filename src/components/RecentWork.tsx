    ...
    <section className="py-24 md:py-32 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-16 animate-on-scroll">
          <p className="text-gold font-semibold text-sm uppercase tracking-[0.2em] mb-4">Recently Completed</p>
          ...
          <p className="text-gray-500 text-lg max-w-lg mx-auto mt-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 stagger-children">
          {recentJobs.map((job, i) => (
            <div
              key={i}
              className="group bg-gray-50 hover:bg-navy-dark rounded-2xl p-6 border border-gray-100 hover:border-white/[0.06] transition-all duration-500"
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-gold/10 group-hover:bg-gold/15 flex items-center justify-center text-gold transition-colors duration-500">
                    {job.icon}
                  </div>
                  <span className="text-sm font-bold text-gray-900 group-hover:text-white transition-colors duration-500">
                    {job.service}
                  </span>
                </div>
                <span className="text-xs text-gray-400 group-hover:text-white/30 transition-colors duration-500">
                  {job.timeAgo}
                </span>
              </div>
              <p className="text-sm text-gray-600 group-hover:text-white/60 leading-relaxed mb-4 transition-colors duration-500">
                {job.detail}
              </p>
              <div className="flex items-center gap-1.5">
                <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 text-gray-300 group-hover:text-gold/50 transition-colors duration-500" fill="none" stroke="currentColor" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                </svg>
                <span className="text-xs font-medium text-gray-400 group-hover:text-gold/60 transition-colors duration-500">
                  {job.area}
                </span>
              </div>
            </div>
